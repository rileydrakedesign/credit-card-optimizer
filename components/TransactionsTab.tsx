        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--surface)]">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-[var(--muted)]">Description</th>
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {sorted.map(t => (
              <tr key={t.id} className={`hover:bg-[var(--surface)] ${t.excluded ? "opacity-50" : ""}`}>
                <td className="px-4 py-3 whitespace-nowrap">{t.date}</td>
                <td className="px-4 py-3 max-w-xs truncate">{t.description}</td>
                <td className={`px-4 py-3 text-right whitespace-nowrap ${t.amount_usd < 0 ? "text-emerald-400" : ""}`}>
                  {t.amount_usd < 0 ? "-" : ""}{formatCurrency(Math.abs(t.amount_usd))}
                </td>
                <td className="px-4 py-3">
                  {t.excluded ? (
                    <span className="text-amber-400 text-xs">{t.exclude_reason}</span>
                  ) : (
                    <select
                      value={t.app_category}
                      onChange={(e) => onCategoryOverride(t.id, e.target.value as AppCategory)}
                      className={`text-sm px-2 py-1 rounded border border-[var(--border)] bg-[var(--surface)] text-[var(--foreground)] ${t.overridden ? "ring-2 ring-emerald-500/50" : ""}`}
                    >
                ...
                <td className="px-4 py-3 text-xs text-[var(--muted)] truncate max-w-[120px]">{t.source_file}</td>
              </tr>
            ))}
          </tbody>
        </table>
